...
namespace edm {
  ...
  namespace impl {
    ...
    template <typename T>
    struct subscript_type<T, std::enable_if_t<not std::experimental::is_array_v<T>>, std::enable_if_t<has_subscript_operator_v<T>>> {
  ...
  public:
    ...
    constexpr propagate_const_array& operator=(propagate_const_array<U>& other) {
      static_assert(std::experimental::is_convertible_v<std::decay_t<U>, std::decay_t<T>>,
